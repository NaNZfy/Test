...
def parser_args():
    ...
    args.output = os.path.join(args.output, args.dataset_name, '%s'%args.img_size, '%s'%args.lb_ratio,  'IAT_%s_%s_%s_%s_%s_%s_%s_%s_%s'%(args.loss_lb, args.loss_ub, args.warmup_epochs, args.threshold, args.cutout, args.lr, args.lb_bs, args.epochs, args.seed))
...
def pseudo_label(ub_train_dataset, model, ema_model, args, logger):
    ...
    pseudo_labels = (outputs>=args.threshold).astype(np.float32)
