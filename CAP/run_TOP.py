...
from dataset.handlers import COCO2014_mask_handler, VOC2012_mask_handler, NUS_WIDE_mask_handler, CUB_200_2011_mask_handler
...
MASK_HANDLER_DICT = {'voc': VOC2012_mask_handler, 'coco': COCO2014_mask_handler, 'nus': NUS_WIDE_mask_handler, 'cub': CUB_200_2011_mask_handler}

NUM_CLASS = {'voc': 20, 'coco': 80, 'nus': 81, 'cub': 312}

TOPK = {'voc': 2, 'coco': 3, 'nus': 2, 'cub': 32}
...
def parser_args():
    ...
    parser.add_argument('--init_neg_per', default=1.0, type=float, 
                        help='init neg_per')
    parser.add_argument('--pos_length', default=1, type=float, 
                        help='the number of epochs for pos_per from init_pos_per to 1.0')
    parser.add_argument('--neg_length', default=1, type=float, 
                        help='the number of epochs for neg_per from init_neg_per to 1.0')
    ...
    args.output = os.path.join(args.output, args.dataset_name, '%s'%args.img_size, '%s'%args.lb_ratio,  'TOP_%s_%s_%s_%s_%s_%s_%s'%(args.loss_lb, args.loss_ub, args.warmup_epochs, args.cutout, args.lr, args.lb_bs, args.epochs))
...
def main_worker(args, logger):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        mAP, CF1, OF1 = validate(val_loader, model, args, logger)
        mAP_ema, CF1_ema, OF1_ema = validate(val_loader, ema_m.module, args, logger)
...
def adjust_per(epoch, args):
    ...
    if epoch==0:
        ...
        args.pos_per += (1-args.init_pos_per)/args.pos_length
        args.neg_per += (1-args.init_neg_per)/args.neg_length
...
def pseudo_label(ub_train_dataset, model, ema_model, args, logger):
    ...
    _, lbs_u_guess = torch.max(outputs_tensor, dim=1)
    pseudo_labels = torch.zeros_like(outputs_tensor).cuda().scatter(1,lbs_u_guess.view(-1,1),1).cpu().numpy()
    

    mask = np.ones_like(pseudo_labels)
...
def validate(val_loader, model, args, logger):
    ...
    outputs_tensor = torch.from_numpy(outputs).cuda()
    topk = torch.topk(outputs_tensor, TOPK[args.dataset_name])
    pseudo_labels = torch.zeros_like(outputs_tensor).cuda().scatter(1,topk.indices,1).cpu().numpy()
